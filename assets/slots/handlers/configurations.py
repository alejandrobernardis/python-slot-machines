...
from  slots.common.utils import datetime_parser
from  slots.machines.configurations import DEVICE_LEVELS, DEFAULT_BETS_ASC

re_gift_time = re.compile(r'\d{1,2}:\d{2}:\d{2}(\.\d+)?')

if settings.DEBUG:
    S3_URL = 'http://dev.domain.tld/static/{}'
else:
    S3_URL = 'http://cdn.domain.tld/static/{}'
...
class DeviceConfiguration(object):
    ...
    _podium = (
        'http://app.domain.tld/static/data/podium/1.json',
    )
