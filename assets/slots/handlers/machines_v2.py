...
import json
import datetime
from  slots.games.roulette import RouletteSlot, RouletteBonus
from  slots.handlers.base import BaseHandler
from  slots.machines.base import AbstractMachine
from  slots.machines.configurations import DEVICE_LEVELS, BONUS_TIME, \
    BONUS_TIMES
from  slots.machines.sims import SimsClient
from  slots.security.sessions import session_verify
from  slots.security.iron_man import IronMan
...
class SpinHandler(BaseHandler):
    ...
    def post(self, *args, **kwargs):
        try:
            ...
            mid = int(mid)
            mid_hash = 'cfg-{}'.format(mid)

            expires = 60 * 60 * 24
            mc = self.application.memory_client
            machine_file = '{}/data/machines/{}.json'.format(
                self.settings.get('static_path'), mid
            )
            machine_data = mc.get(mid_hash)

            if not machine_data:
                with open(machine_file, 'r') as file_input:
                    machine_data = json.load(file_input)
                mc.save(mid_hash, machine_data, expires)

            # group = choice(machine_data.get('groups', []))
            group = SimsClient.get_factor(mid, lines, bet)

            if not group:
                raise ValueError('Group (x)')

            machine_factor_name = 'cfg-{}-factor-{}'.format(mid, group)
            machine_factor_file = '{}/data/machines/{}/{}.json'.format(
                self.settings.get('static_path'), mid, group
            )
            machine_factor_data = mc.get(machine_factor_name)

            if not machine_factor_data:
                with open(machine_factor_file, 'r') as file_input:
                    machine_factor_data = json.load(file_input)
                mc.save(machine_factor_name, machine_factor_data, expires)

            matrix = choice(machine_factor_data.get('values', []))

            if not matrix:
                raise ValueError('Matrix (x)')
            ...
            machine.spin(
                ...
                {
                    'lines': matrix,
                    'rails': machine_data.get('rails'),
                    'paytable': machine_data.get('paytable'),
                    'max_lines': machine_data.get('max_lines'),
                    'mini_game': machine_data.get('mini_game')
                },
            )
...
class SpinMiniGameGetHandler(BaseHandler):
    ...
    def post(self, *args, **kwargs):
        try:
            ...
            try:
                module = import_module(' slots.machines.slots')
            except ImportError:
...
handlers_list = [
    (r'/do/v2.0/spin/?', SpinHandler),
    (r'/do/v2.0/spin/bonus/?', SpinBonusHandler),
    (r'/do/v2.0/spin/game/?', SpinGameHandler),
    (r'/do/v2.0/spin/mini/game/get/?', SpinMiniGameGetHandler),
    (r'/do/v2.0/spin/mini/game/save/?', SpinMiniGameSaveHandler),
]
